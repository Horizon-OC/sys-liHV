...
u8 I2c_Bq24193_Convert_mV_Raw(u32 mv) {
    if (mv < BQ24193_VREG_MIN_MV)
        mv = BQ24193_VREG_MIN_MV;
    else if (mv > BQ24193_VREG_MAX_MV)
        mv = BQ24193_VREG_MAX_MV;

    /* Round down to 16 mV step */
    mv -= (mv - BQ24193_VREG_MIN_MV) % BQ24193_VREG_STEP_MV;

    u8 vreg = (mv - BQ24193_VREG_MIN_MV) / BQ24193_VREG_STEP_MV;

    /* VREG goes into bits 7–2 */
    return (vreg & 0x3F) << 2;
}

u32 I2c_Bq24193_Convert_Raw_mV(u8 raw) {
    u8 vreg = (raw >> 2) & 0x3F;
    return BQ24193_VREG_MIN_MV + (vreg * BQ24193_VREG_STEP_MV);
}

Result I2c_Bq24193_GetChargeVoltageLimit(u32 *mv) {
    u8 raw;
    Result res = I2cRead_OutU8(
        I2cDevice_Bq24193,
        BQ24193_CHARGE_VOLTAGE_CONTROL_REG,
        &raw
    );
    if (res)
        return res;

    *mv = I2c_Bq24193_Convert_Raw_mV(raw);
    return 0;
}

Result I2c_Bq24193_SetChargeVoltageLimit(u32 mv) {
    u8 old_raw;
    Result res = I2cRead_OutU8(
        I2cDevice_Bq24193,
        BQ24193_CHARGE_VOLTAGE_CONTROL_REG,
    ...
    u8 new_raw = I2c_Bq24193_Convert_mV_Raw(mv);

    /* Preserve bits 1:0 */
    new_raw |= (old_raw & 0x03);
